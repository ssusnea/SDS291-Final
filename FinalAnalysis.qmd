...
plot1 <- ggplot(data = pred_datamath_teacherexp, aes(x = teacher_experience, y = pred_prob)) +
  geom_ribbon(aes(ymin = pred_lower, ymax = pred_upper), fill = "blue", alpha = 0.50) +
  geom_line() +
  labs(title = "Teachers' Experiences vs. Math Scores",
       x = "Percentage of Teacher With 3+ Year Experiences",
...
## Model 2: English
For our model predicting a school's average 2018 English Regents score only the coefficient estimate for chronic absenteeism was statistically significant and negatively associated with the dependent variable. As the percentage of chronic absenteeism within a student body increases, a school's average score for the 2018 English Regents decreases (see @fig-plot2). The other predictors included in this model were not statistically significant so our initial hypothesis that average English test scores have a relationship with teacher experience and a school's supportive environment was not supported by this model.
...
```{r, echo=FALSE}
#| label: fig-plot2
#| fig-cap: "Model 2 Predicting 2018 English Regents Score"

plot3 <- ggplot(data = pred_dataenglish, aes(x = chronically_absent, y = pred_prob)) +
...
## we noticed that these residuals are roughly normally distributed 
vif(lm_english)
## vif looks fine
...
# chronically absent vs mean english was the only scatter plot to look remotely
# linear.
...
summary(lm_math)
# for model 2 we noticed that the intercept, teacher_experience, and
# chronically_absent were statistically significant.

# condition checks: 
plot(lm_math, which = 1)
## these points are generally linear and have equal variance 
