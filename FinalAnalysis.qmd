...
## Model 1: Math
From the model predicting a school's average 2018 Algebra 2 Regents score, two statistically significant variables emerge as predictors of math test scores for a school. 

First, economic need is negatively associated with a school's average Algebra 2 Regents score. This implies that as the percentage of students with economic need increases, students' math scores decrease (see @fig-plot1 (a)). However, the school's supportive environment, the other variable mentioned in our initial hypothesis, did not exhibit statistical significance in this model. We failed to provide evidence for an association between the school's supportive environment and students' math scores. Nevertheless, the coefficient for the control variable "chronic absence" is also statistically significant. There is a negative association between chronic absence and students' math scores, indicating that as students' absences increase, their math scores decrease (see @fig-plot1 (b)).

The adjusted $R^2$ for this model is $47.7$%, signifying that the model explains a moderate proportion of the variability in students' math scores, leaving room for improvement. The model's F-statistic is also statistically significant, indicating that it successfully reduces the unexplained variation in students' math scores by incorporating four different variables.
...
## Model 2: English
For our model predicting a school's average 2018 English Regents score three of our predictors were statistically significant: chronic absenteeism, economic need, and rigorous instruction. Both chronic absenteeism and economic need have a negative relationship with a school's average score for the 2018 English Regents (see @fig-plot2) while rigorous instruction has a positive relationship. The other predictors included in this model were not statistically significant, so our initial hypothesis that average English test scores have a relationship with teacher experience as well as a school's supportive environment was not supported by this model.
...
# Conclusion
For this study, we wanted to explore if a school's average Algebra 2 and English Regents scores are significantly associated with a school's supportive environment, teachers' teaching experience, the percentage of rigorous instruction, students' absenteeism rates, and the percentage of students with economic need. 

Our analysis does not supports our primary hypothesis. In our first model, with the schools' average Algebra 2 Regents score as the dependent variable, we failed to reject the null hypothesis for both the teacher experience and supportive environment predictors. However, we reject the null hypothesis for economic need and chronic absenteeism, indicating that there is a relationship between a school's average Algebra 2 Regents score and chronic absenteeism and economic need. We can confidently conclude that both the chronic absenteeism and economic need variables are statistically significant across both models.

Moving on to the second model where the dependent variable is a school's average English Regents score, we rejected the null hypothesis for chronic absenteeism, economic need, and rigorous instruction. This indicates there is a significant relationship between the average English Regents score in schools and these three variables, but there is not a significant relationship between average English Regents scores and our key variables teacher experience, and school supportive environment.
...
vif(lm_english)
## vif looks fine although chronic absence has a vif of 3.36 and economic need and supportive environment have vif scores of 2 and 2.5 respectively
...
# chronically absent vs mean english looked somewhat linear, economic need vs mean english 
# was linear. 
...
vif(lm_math)
## vif looks fine although chronic absence has a 3.3 vif score and both economic need and supportive environment have vif over 2
...
# linear. economic need vs mean algebra2 was mostly linear till the right most end 
# where it became quadratic
