...
# Conclusion
For this study, we would like to see whether math test scores and English score are significantly associated with a school supportive environment and teachers' teaching experience, the percentage of rigorous instruction, and students' absenteeism rates. 

Our analysis does not supports our primary hypothesis. In our initial model, where the dependent variable is the English score, we fail to reject the null hypothesis for all independent variables except the factor of chronic absenteeism. This indicates there is a significant relationship between the average English scores in schools and the percentage of students experiencing chronic absenteeism.

Moving on to the second model, with the math score as the dependent variable, we fail to reject the null hypothesis for teacher experience and the supportive environment factor. However, we reject the null hypothesis for rigorous instruction and chronic absenteeism, indicating that there is a relationship between average math score and chronic absenteeism and rigorous teaching. We can successfully conclude that the chronically absent factor is statistically significant across both models.

# Limitation:
In our research, we selected only four variables as predictor variables, omitting consideration of other potential factors such as school race composition and overall school quality that could have an impact on our results. This selective approach may have led to an oversimplification of our sample, as we focused solely on students' performance measured by average math and English scores. 

Furthermore, it's essential to note that we combined two distinct datasets, since one dataset only included students' grades before entering high school. This merging of datasets introduces potential complexities and nuances that should be acknowledged in interpreting our findings.

Since we also fail to pass the regression condition first, we need to be very cautious about making any causal inference based on this result since the result is not technically valid. This problem can be addressed by the possibility to selecting a data with more observations, and be more careful in selecting predicting factors. By making these adjustments, we can enhance the quality of our research and increase the accuracy of our findings.
