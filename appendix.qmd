...
We analyze our results by conducting an multiple regression test. In our initial model, where the dependent variable is the English score, we fail to reject the null hypothesis for all independent variables except the factor of chronic absenteeism. This indicates there is a significant relationship between the average English scores in schools and the percentage of students experiencing chronic absenteeism.

Moving on to the second model, with the math score as the dependent variable, we fail to reject the null hypothesis for teacher experience and the supportive environment factor. However, we reject the null hypothesis for rigorous instruction and chronic absenteeism, indicating that there is a relationship between average math score and chronic absenteeism and rigorous teaching. We conclude that the chronically absent factor is statistically significant across both models.

Since we also fail to pass the regression condition first, we need to be very cautious about making any causal inference based on this result since the result is not technically valid. This problem can be addressed by the possibility to collect more data, and be more careful in selecting predictors factors. By making these adjustments, we can enhance the quality of our research and increase the accuracy of our findings.
